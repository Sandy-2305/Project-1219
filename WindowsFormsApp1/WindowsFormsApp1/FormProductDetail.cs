...
namespace WindowsFormsApp1
{
    public partial class FormProductDetail : Form
    {
        ...
        {
            if ((txtProductName.Text != "") && (txtProductPrice.Text != "") && (txtProductDetail.Text != "") && (pictureBoxProduct.Image != null))
            {
                if (isPicUpdate == true)
                {
                ...
                SqlConnection con = new SqlConnection(strMyPJDBConnectionString);
                con.Open();
                string strSQL = "update products set ProductName=@NewPname, ProductPrice=@NewPrice, ProductInfo=@NewPDesc, ProductImage=@NewPimage where ProductID=@SearchID;";
                SqlCommand cmd = new SqlCommand(strSQL, con);
                cmd.Parameters.AddWithValue("@SearchID", ID);
